...
GetOptions(
    ...
    "outdir=s" => \$outdir,
    "conversion_file=s" => \$sample_conv_file, 
    "trim_files=s{,}" => \@primer_trim,
    "qsub",
    "print_scripts",
    "help",
) or die "Syntax error!\n";
...
my %sample_conversions = ();
if ($sample_conv_file){
    open (my $CONV, $sample_conv_file) or die "Could not open $sample_conv_file: $!\n";
    while (my $line = <$CONV>){
        chomp $line;
        my @s = split("\t", $line);
        if (exists $sample_conversions{$s[0]}){
            die "Duplicate sample in $sample_conv_file: $s[0]\n";
        }
        next if $s[1] =~ /N\/A/; 
        next if not $s[1];
        $sample_conversions{$s[0]} = $s[1];
    }
}
...
=cut
    ...
    $sample = $dirs[-1]; 
    $snum = $sample;
    if (exists $sample_conversions{$sample}){
        $sample = $sample_conversions{$sample};
    }
    if ($f =~ /(\d{6})_\w{5,6}_\w{4}_\w{10}/){
        $date = $1;
    }elsif($f =~ /(\d{6})_/){
        $date = $1;
    }else{
    ...
    my $unique_date_snum = check_sample_name($sample, "$date-$snum", $lane, $read, $d, $f); 
    $fastqs{$sample}->{"$unique_date_snum"}->{$lane}->{$read} = "$d$f";
}
my @bams = ();
my @scripts = ();
foreach my $s (sort keys %fastqs){
    foreach my $d (sort keys %{$fastqs{$s}}){
        foreach my $l (sort keys %{$fastqs{$s}->{$d}}){
            ...
            my $dir;
            my ($fq_1, $fq_dir) = fileparse($reads[0]); 
            my $fq_2 = @reads > 1 ? fileparse($reads[1]) : ""; 
            if ($outdir){
                $dir = $outdir;
            }else{
                $dir = $fq_dir;
            ...
            if (@primer_trim){ 
                my $trimmed_dir = "$fq_dir/trimmed";
                if (not -d "trim_fastqs"){
                    mkdir("trim_fastqs") or die "Failed to make trim_fastqs directory: $!\n";
                }
                if (not -d $trimmed_dir){
                    mkdir($trimmed_dir) or die "Failed to make trimmed fastqs directory $trimmed_dir for $s: $!\n";
                }
                my $adapt_string = "-a file:$primer_trim[0] "; #3' adaptor file first
                $adapt_string .= "-A file:$primer_trim[0] " if @reads > 1; 
                if (@primer_trim > 1){  #then optional 5' adaptor file
                    $adapt_string .= "-g file:$primer_trim[1] "; 
                    $adapt_string .= "-G file:$primer_trim[1] " if @reads > 1; 
                }
                my $reads_in = join(" ", @reads); 
                my $out_string = "-o $trimmed_dir/$fq_1 ";
                $out_string .= "-p $trimmed_dir/$fq_2 " if $fq_2; 
                my @trimmed_reads = ();
                push @trimmed_reads, "$trimmed_dir/$fq_1"; 
                push @trimmed_reads, "$trimmed_dir/$fq_2" if $fq_2;
                @reads = @trimmed_reads;
                my $script = "trim_fastqs/trim-$s-$d-$l.sh";
...
# Load modules
module load apps/gcc/python/2.7.6
cutadapt $adapt_string $reads_in $out_string
...
sub check_sample_name{
    ...
    my $rename = rename_sample($date); 
    if ($fastqs{$sample}->{$date}){
        if ($fastqs{$sample}->{$date}->{$lane}){
            if ($fastqs{$sample}->{$date}->{$lane}->{$read}){
                warn "WARN: File $d/$f is a duplicate date/id ($date) lane ($lane), read ($read) and sample ($sample). Trying date/id name of $rename...\n";
                return check_sample_name($sample, $rename, $lane, $read, $d, $f);
            }else{
                return $date ;#other read pair for sample - keep sample name
            }
        }
...
sub usage{
    ...
    -c    --conversion_file [optional sample name conversion CSV file]
    -t    --trim_files      [optional adaptor/primer fasta sequence file(s) for trimming with cutadapt. First file will be used for trimming 3' adaptor/primer sequences, an optional second file will be used for trimming 5' adaptor/primer sequences.
